...
#	return output
    ...
    output.append(str(positive_sentiment))
    output.append(str(neutral_sentiment))
    output.append(str(negative_sentiment))
    verdict = get_verdict(sentiments)
    ...
    with open("calculate_polarity.txt", "r") as txt:
        ...
        for x in txt:
            ...
            for k in range(len(taggeddata)-2):
                ...
                value = check(taggeddata[k][1],
                              taggeddata[k+1][1], taggeddata[k+2][1])
                if(value > 0):
                    pattern = []
                    pattern.append(
                        "".join(taggeddata[k][0])+" "+"".join(taggeddata[k+1][0]))
                    pattern.append("\n")
